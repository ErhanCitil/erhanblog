...
1. ``erhanblog-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``erhanblog-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``erhanblog-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``erhanblog-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``erhanblog-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``erhanblog-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``erhanblog-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``erhanblog-pr-isort``: checks that imports are correctly
